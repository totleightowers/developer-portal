...
source_path: concepts/networking/networking-approach.html.md.erb
ingested_at: "2026-02-27T16:18:17.712Z"
---
...
We looked into having a [VPC](https://docs.aws.amazon.com/vpc/latest/userguide/what-is-amazon-vpc) per account. While this provides good network isolation, because we have one account per application environment, it would mean a lot of VPC peering would be required to connect applications together if needed.
...
These VPCs are then shared via [Resource Access Manager (RAM)](https://docs.aws.amazon.com/ram/latest/userguide/what-is) to the application accounts.

For example the production LAA VPC may be shared to multiple LAA application accounts. This enables LAA applications to communicate with each other without the need for [VPC peering](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-peering) as they are using the same VPC.

Connection to other VPCs - i.e. if an LAA application needs to communicate with a HMPPS application - is done through the [Transit Gateway](#what-we-decided-transit-gateway). [NACLs](#nacls) can be opened to allow access, and [security groups](https://docs.aws.amazon.com/vpc/latest/userguide/VPC_SecurityGroups) from other accounts can be referenced.
...
For more information on the subnet ranges, see the [subnet allocation](subnet-allocation) page.

##### Protected

Protected subnets are created per account and used for [VPC endpoints](https://docs.aws.amazon.com/vpc/latest/privatelink/vpc-endpoints). We provide the following VPC endpoints as standard, and can add additional ones as required:
...
Access to the subnet sets is controlled with [Network Access Control Lists (NACLs)](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-network-acls).
...
AWS [Network Firewalls](network-firewall) provide additional controls to traffic entering and exiting the Modernisation Platform.
...
The [transit gateway](https://docs.aws.amazon.com/vpc/latest/tgw/what-is-transit-gateway) allows us to interconnect the different VPCs to enable communication between business units. The Transit Gateway also enables connection to third parties external to the MoJ via a [VPN](https://docs.aws.amazon.com/vpc/latest/userguide/vpn-connections).
...
For more information on how the VPC sharing fits into the bigger networking picture, see the main [networking diagram](networking-diagram)
