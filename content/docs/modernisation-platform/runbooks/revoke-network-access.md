...
source_path: runbooks/revoke-network-access.html.md.erb
ingested_at: "2026-02-27T16:18:17.746Z"
---
...
1. Visit the [AWS SSO page](https://moj.awsapps.com/start#/) and select the target environment
...
3. Copy the `Option 1: Set AWS environment variables` details into your terminal
    ...
    ```
4. Run the `quarantine-security-groups.sh` [script](https://github.com/ministryofjustice/modernisation-platform/blob/main/scripts/internal/quarantine-security-groups.sh) and follow the prompts to remove all security group rules
5. The script creates text backups of all security group rules before any changes, and logs the entire process to a timestamped log file for auditing and potential recovery
...
Due to the way Network Firewall applies rules in AWS (see [here](https://docs.aws.amazon.com/network-firewall/latest/developerguide/suricata-rule-evaluation-order) ), this section only covers changes to the VPC NACL.
...
Due to the way Network Firewall applies rules in AWS (see [here](https://docs.aws.amazon.com/network-firewall/latest/developerguide/suricata-rule-evaluation-order) ), this section only covers changes to the VPC NACLs.
