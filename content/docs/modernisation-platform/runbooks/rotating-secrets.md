...
source_path: runbooks/rotating-secrets.html.md.erb
ingested_at: "2026-02-27T16:18:17.748Z"
---
...
We don't have many secrets on stored on the Modernisation Platform, but they are rotated regularly, typically every 180 days.
...
| Slack Webhooks                                    | `slack_webhooks`                        | Used for sending notifications to specified Slack channels when environment JSON files are modified                                                                                                                                                                                                            | AWS Secrets Manager     | Use this [runbook](https://user-guide.modernisation-platform.service.justice.gov.uk/runbooks/rotating-secrets.html#slack-webhook-url-or-slack-webhooks) to rotate the secret                                                                               | 180            |
| Slack Webhook URL                                 | `slack_webhook_url`                        | Slack channel modernisation-platform-notifications webhook url for sending notifications to slack                                                                                                                                                                                                            | AWS Secrets Manager     | Use this [runbook](https://user-guide.modernisation-platform.service.justice.gov.uk/runbooks/rotating-secrets.html#slack-webhook-url-or-slack-webhooks) to rotate the secret                                                                               | 180            |
| GitHub MP CI User PAT                             | `github_ci_user_pat`                       | Used to create PRs etc in GitHub actions and deploy GitHub resources via Terraform                                                                                                                                                      | AWS Secrets Manager     | Use this [runbook](https://user-guide.modernisation-platform.service.justice.gov.uk/runbooks/rotating-secrets.html#github-mp-ci-user-pat-or-github-mp-ci-user-environments-repo-pat-or-github-scim-to-aws-pat) to rotate the secret | 180            |
...
| GitHub SCIM to AWS PAT                            | `aws_saml`                                 | Used by SCIM Lambda job to synchronize GitHub organization teams and members with AWS IAM Identity Center.                                                                                                                              | AWS Secrets Manager (MOJ master) | Use this [runbook](https://user-guide.modernisation-platform.service.justice.gov.uk/runbooks/rotating-secrets.html#github-mp-ci-user-pat-or-github-mp-ci-user-environments-repo-pat-or-github-scim-to-aws-pat) to rotate the secret         | 90             |
...
| Modernisation Platform Github App Private Key     | `modernisation_platform_github_app_private_key` | Used by the github workflows that require read & write access to our repositories and org-related assets such as teams                                                                                                             | AWS Secrets Manager     | Use this [runbook](https://user-guide.modernisation-platform.service.justice.gov.uk/runbooks/rotating-secrets.html#Modernisation Platform Github App Private Key) to rotate the secret                                                   | 180            |



## Runbooks

### GitHub MP CI User PAT, GitHub MP CI User Environments Repo PAT, GitHub SCIM or AWS PAT.

This runbook describes the process for rotating the **github_ci_user_pat** or **github_ci_user_environments_repo_pat** or **github_scim_to_aws_pat** secrets.
...
6. Navigate to AWS Secrets Manager [github_ci_user_pat](https://eu-west-2.console.aws.amazon.com/secretsmanager/secret?name=github_ci_user_pat&region=eu-west-2) or [github_ci_user_environments_repo_pat](https://eu-west-2.console.aws.amazon.com/secretsmanager/secret?name=github_ci_user_environments_repo_pat&region=eu-west-2) or [aws_saml](eu-west-2.console.aws.amazon.com/secretsmanager/secret?name=aws_saml&region=eu-west-2) secret and click `Retrieve secret value`
...
10. After updating github token for aws_saml secret in MOJ Master account, run the [management-account-apply.yml](https://github.com/ministryofjustice/aws-root-account/actions/workflows/management-account-apply.yml) workflow in the aws-root-account repository. Select last successful run, and re-run the jobs so the Auth0 Tenant is rebuilt with the new secret.

### Modernisation Platform Github App Private Key.

This covers the steps to create a new private key for the app and add that to the secret.

1. Navigate to the github app in the Modernisation Platform organisation link - https://github.com/organizations/ministryofjustice/settings/apps/modernisation-platform
2. In the section headed `Private Keys` select the option `Generate a Private Key`.
3. The new key will be downloaded via the browser into the Downloads folder of the device. Make a note of the file name.
4. Log onto the Modernisation Platform account as ModernisationPlatformEngineer and in secrets manager, navigate to the secret named `modernisation_platform_github_app_private_key`.
5. Using a tool such as `pbcopy`, select the contents of the private key from the downloaded file.
6. Edit the secret in Secrets Manager and paste the new value for the secret and save the changes. This will create a new version of the secret.
7. Run one of the workflows that uses the secret e.g. `format-code.yml` to test the change.
8. If the test is successful, delete the original private key.
9. In the event of an issue, rollback the version of the secret by following the steps outlined in this doc - https://docs.aws.amazon.com/secretsmanager/latest/userguide/roll-back-secret.html. The newer key should then be deleted, retaining only the original key.

### Slack Webhook URL or Slack Webhooks

This runbook describes the process for rotating the **slack_webhook_url** and **slack_webhooks** secrets.
> **NB** The slack webhook for the modernisation-platform channel needs to be updated in both secrets. Note that the `slack_webhooks` secret is a list and so you should only update the **Secret value** for `modernisation-platform`
...
5. Navigate to AWS Secrets Manager [slack_webhook_url](https://eu-west-2.console.aws.amazon.com/secretsmanager/secret?name=slack_webhook_url&region=eu-west-2) and [slack_webhooks](https://eu-west-2.console.aws.amazon.com/secretsmanager/secret?name=slack_webhooks&region=eu-west-2) secret and click `Retrieve secret value`
6. Click `Edit` and replace the secret value with the new one and click `Save`
7. Delete the previous webhook for the `modernisation-platform` from the `Incoming webhooks` list in Slack
...
#### 2.  **Locate the Application or Service**
   ...
   - Search for the relevant application, such as `justicedigital-panda-awsidentitycenter` or `justicedigital-panda-awsidentitycenter-scim.
...
#### 6. **Verify Functionality**
   ...
   - After updating a secret, always run the [`management-account-apply.yml`](https://github.com/ministryofjustice/aws-root-account/actions/workflows/management-account-apply.yml) workflow. Select last successful run, and re-run the jobs to propagate changes. Test the application or service to confirm that it works with the new secret. 
...
1. Get in touch with the Monitoring and Integration Team via their team email address (see [runbook doc](integration-with-protective-monitoring)) and confirm a new access key has been created.
