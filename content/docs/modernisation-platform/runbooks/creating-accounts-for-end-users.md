...
source_path: runbooks/creating-accounts-for-end-users.html.md.erb
...
All new account requests will be added to the **To Do** column of the [Kanban board](https://github.com/orgs/ministryofjustice/projects/17/views/48) within the Modernisation Platform.
...
- Update the relevant files in the `policies` directory.  
...
In a recent update to the JSON environment file we have added a new tag called critical-national-infrastructure, this tag exists to help us report on
...
This stage sets up [Resource Access Manager (RAM) sharing](https://docs.aws.amazon.com/ram/latest/userguide/what-is) to share the relevant business unit subnet sets to the application account.

Business unit VPCs and subnet sets are already created for all the MoJ business units. If an additional VPC or subnet set is needed, see guidance [here.](https://user-guide.modernisation-platform.service.justice.gov.uk/runbooks/creating-vpcs).
